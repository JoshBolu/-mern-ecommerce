...
const generateTokens = (userId, userName) => {
    const accessToken = jwt.sign({userId, userName}, process.env.ACCESS_TOKEN_SECRET, {expiresIn: '15m'})
    const refreshToken = jwt.sign({userId, userName}, process.env.REFRESH_TOKEN_SECRET, {expiresIn: '7d'})
...
export const refreshToken = async(req, res) => {
    try{
        ...
        res.status(StatusCodes.OK).json({accessToken, message: "Access token refreshed successfully"})
    }
    catch(err){
